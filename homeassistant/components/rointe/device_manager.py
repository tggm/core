...
class RointeDeviceManager:
    ...
    async def update(self) -> dict[str, list[RointeDevice]]:
        ...
        LOGGER.debug("Processing zones")

        for zone_key in installation["zones"]:
            LOGGER.debug("Processing zone: %s", zone_key)
            ...
            for device_id in devices:
                LOGGER.debug("Processing device ID: %s", device_id)

                if not devices[device_id]:
                    LOGGER.debug("Device ID: %s has no data", device_id)
    ...
    def _add_or_update_device(
        ...
        if device_id in self.rointe_devices:
            ...
            try:
                ...
                LOGGER.error("Unable to process device %s. Error: %s", device_id, ex)
